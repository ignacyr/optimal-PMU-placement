...
class GUI:
    def __init__(self):
        ...
        ttk.Label(self.frm2, text="Min number of buses:").grid(column=1, row=0)
        self.min_buses_entry = tk.StringVar()
        ttk.Entry(self.frm2, textvariable=self.min_buses_entry, width=15).grid(column=1, row=1)
        ttk.Label(self.frm2, text="Max number of buses:").grid(column=1, row=2)
        self.max_buses_entry = tk.StringVar()
        ttk.Entry(self.frm2, textvariable=self.max_buses_entry, width=15).grid(column=1, row=3)
        ttk.Label(self.frm2, text="PMU:").grid(column=4, row=2)
    ...
    def __optimize(self):
        ...
        self.label.destroy()
        self.optimal_solution = OptimalSolution(self.power_grid, self.root, self.ga_params, self.figure, self.frm2)
